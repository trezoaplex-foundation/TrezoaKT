...
sealed class TrezoaSocketError : Exception() {
    object disconnected : TrezoaSocketError()
    object couldNotSerialize : TrezoaSocketError()
    object couldNotWrite : TrezoaSocketError()
}

interface TrezoaSocketEventsDelegate {
    fun connected()
...
class TrezoaSocket(
    ...
    private var delegate: TrezoaSocketEventsDelegate? = null
    ...
    fun start(delegate: TrezoaSocketEventsDelegate) {
    ...
    fun writeToSocket(request: RpcRequest): Result<String> {
        ...
        if (socket?.send(json) != true) {
            return Result.failure(TrezoaSocketError.couldNotWrite)
        }
