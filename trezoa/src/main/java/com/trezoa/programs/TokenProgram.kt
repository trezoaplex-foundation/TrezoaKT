...
object TokenProgram : Program() {
    ...
    /**
     * Transfers an TPL token from the owner's source account to destination account.
     * Destination pubkey must be the Token Account (created by token mint), and not the main TRZ address.
     * @param source TPL token wallet funding this transaction
     * @param destination Destined TPL token wallet
    ...
    fun transferChecked(
        ...
        val keys: MutableList<AccountMeta> = ArrayList()
        keys.add(AccountMeta(source, false, true))
        // index 1 = token mint (https://docs.rs/tpl-token/3.1.0/tpl_token/instruction/enum.TokenInstruction.html#variant.TransferChecked)
